...
&ensp;`http://www.opengis.net/def/crs/{authority}/{version}/{code}`  
&ensp;where `{authority}` designates the authority responsible for the definition of the crs (usually "EPSG" or "OGC"), and `{version}`  
&ensp;designates the specific version of the crs ("0" (zero) is used if there is no version).
