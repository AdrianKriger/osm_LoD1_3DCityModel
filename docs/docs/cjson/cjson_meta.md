...
enspwhere `{authority}` designates the authority responsible for the definition of the crs (usually "EPSG" or "OGC"), and `{version}`  
enspdesignates the specific    version of the crs ("0" (zero) is used if there is no version).
