...
a) [PyVista](https://www.pyvista.org/) is [built-in](https://github.com/AdrianKriger/osm_LoD1_3DCityModel/blob/main/osm3DCode.py#L440-L466) and will execute after the triangulation; before the 3D City Model is created. In addition to verifying the building footprints have been removed from the surface you might encounter a spike or two. Generally if you trace this to its root *---and open in an osm editor---* you'll discover buildings crossing each other. The triangulation knows there are lines and expects a vertex where they intersect. There is none and the result is a spike. Drop a node to correct the topology. *We are transforming volunteered public data into a value-added product. Alchemy is a process. Be patient.*
